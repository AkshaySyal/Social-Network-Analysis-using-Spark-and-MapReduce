...
aws.bucket.name=cs6240-spark-bucket-hw3
aws.input=input
aws.output=output
aws.log.dir=log
aws.num.nodes=8
aws.instance.type=m3.xlarge
aws.cluster.name=Triangle-Count-Spark-Cluster-RepRDD-10,000-8
# -----------------------------------------------------------
...
local: jar clean-local-output
	spark-submit --class ${job.name} --master ${local.master} --name "${app.name}" ${jar.name} ${local.input} ${local.output} ${local.MAX}
...
aws: jar upload-app-aws delete-output-aws 
	aws emr create-cluster \
		...
		--steps Type=CUSTOM_JAR,Name="${app.name}",Jar="command-runner.jar",ActionOnFailure=TERMINATE_CLUSTER,Args=["spark-submit","--deploy-mode","cluster","--class","${job.name}","s3://${aws.bucket.name}/${jar.name}","s3://${aws.bucket.name}/${aws.input}","s3://${aws.bucket.name}/${aws.output}","${local.MAX}"] \
...
download-log-aws: clean-local-output
	mkdir ${local.log}
	aws s3 sync s3://${aws.bucket.name}/${aws.log.dir} ${local.log}

# # Download everything from S3. Deletes output that was downloaded in the command above
# download-bucket: clean-local-output
# 	mkdir ${local.output}
